...
public class DeletePlanServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (txtPlanId == null || txtPlanId.isEmpty()) {
                response.sendRedirect("personalPlanPage");
            }
            int planId = 0;
            try {
                planId = Integer.parseInt(txtPlanId);
            } catch (Exception e) {
                response.sendRedirect("personalPlanPage");
            }
            CustomerPlanDAO planDao = new CustomerPlanDAO();
            planDao.disablePlan(account.getId(), planId);
            response.sendRedirect("personalPlanPage");

        } catch (SQLException ex) {
            Logger.getLogger(DeletePlanServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
